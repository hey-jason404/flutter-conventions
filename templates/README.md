...
`shared/flutter-conventions/` 是模板的預設 path（最 generic、撞名機率最低）。依你的 repo 結構可選別的：
...
選定後**所有模板裡的 path 都要對應修改**（步驟 3 會做）。
...
- **如果**你 mount 在非預設位置（例如 `src/docs/conventions/`），把模板內所有 `shared/flutter-conventions/` 改成你實際的 path
...
```bash
git submodule set-url shared/flutter-conventions https://github.com/hey-jason404/flutter-conventions.git
git submodule sync
git submodule update --init --recursive
git add .gitmodules
...
### 2. Clone / 切 branch 後 submodule 資料夾是空的
...
```bash
ls shared/flutter-conventions/conventions.md   # 必須存在
```
...
3. submodule 是否確實 init（`ls shared/flutter-conventions/conventions.md` 應該看到檔）
